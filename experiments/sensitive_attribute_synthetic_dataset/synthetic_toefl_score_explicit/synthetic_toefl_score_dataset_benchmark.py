...
class ArtificialTOEFLScoreExplicitDataset(NumericalConditionDatasetBuilderBase):
    def __init__(
        self,
        csv_data_filepath,
    ...
    def generate_corpus_text(
        ...
        name = metadata["name"]
        gender = "male" if metadata["gender"] == "M" else "female"
        country_code = metadata["country_code"]

        coutry_name = pycountry.countries.get(alpha_2=country_code).name
        ...
        corpus_text = f"{name} is a {gender} working in a {industry_text} company with a {skill_text} job, and the TOEFL score is {number_text} in {coutry_name}."
