...
class ArtificialSalaryTable:
    ...
    def generate_csv(
        ...
        salary_range=(700000000, 2000000000),
        salary_step=10000000,
        seed=42,
    ):
        logger.info(
            f"Generating artificial salary dataset... ({num_category_pairs} skill-industry-salary pairs x {len(country_codes)} countries x genders = {num_category_pairs * len(country_codes) * 2})"
        )
        ...
        skill_industry_salary_pairs = cls._make_skill_industry_salary_pairs(
            skills,
            industries,
        ...
        rows = []
        for index, pairs in enumerate(skill_industry_salary_pairs):
            skill, industry, salary, salary_k, salary_m = pairs
    ...
    @staticmethod
    def _make_skill_industry_salary_pairs(
        skills: list[str],
        industries: list[str],
        n_pairs: int = 100,
        salary_range=(700000000, 2000000000),
        ...
        for skill, industry in selected_skill_industry_pairs:
            salary = faker.random_int(
                min=salary_range[0], max=salary_range[1], step=salary_step
            )
            salary_k = ceil(salary / 1000)
            salary_m = ceil(salary / 1000000)
            result.append((skill, industry, salary, salary_k, salary_m))
